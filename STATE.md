...
| **Remaining on `/subjects/[id]`** | Overview brief fully ported. Minor leftovers: the lazy "show full quote" expand on evidence cards (currently shows the cross-analyzer excerpt only — `getPromptResponses` already wired for it) + a refresh-history disclosure. **Next structural step: the subject-scoped spoke sidebar** (nested `subjects/[id]` layout) — the unblock before porting the other spokes (Visibility, Narrative, Competition, Sources, Prompts, Recommendations). |
