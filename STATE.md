...
| **Remaining** | Overview brief + spoke sidebar done. **Visibility spoke FIRST SLICE shipped** (`subjects/[id]/visibility/page.tsx`: per-platform performance table + answer-prominence rank distribution + per-prompt coverage grid; `SPOKES.visibility` flipped to `built:true`). Visibility deferred: platform×topic heatmap, rank-dist-by-platform/topic dropdowns, cross-platform divergence (all in payload). **Competitive spoke FIRST SLICE shipped** (`subjects/[id]/competition/page.tsx`: mention-rate leaderboard + first-mention steal share + co-mention frequency/mention-quality; `SPOKES.competition` → `built:true`). Competitive deferred: per-platform entity-SoV grid, topic battleground (`topic_leaderboard`), per-platform landscape. **Narrative spoke FIRST SLICE shipped** (`subjects/[id]/narrative/page.tsx`: sentiment mix (pos/neu/neg stacked bar) + dominant AI framings (clusters) + sentiment-by-topic matrix; `SPOKES.narrative` → `built:true`). Narrative deferred: per-platform sentiment, narrative-score trajectories (directional_lean / criticism_severity / certainty / net_sentiment). **Sources spoke FIRST SLICE shipped** (`subjects/[id]/sources/page.tsx`: source type mix aggregated by category + enriched citation leaderboard with per-platform chips inline; `SPOKES.sources` → `built:true`). Sources deferred: full per-platform source matrix, domain-category controls. **Prompts spoke shipped** (`subjects/[id]/prompts/page.tsx` + client `PromptCard.tsx`: tracked-prompt list with per-platform status chips + lazy "Show responses" expand that fetches full per-platform response text via the `/api/subjects/{id}/prompts/{promptId}/responses` proxy — FIRST interactive spoke; `SPOKES.prompts` → `built:true`). **ALL 6 spokes done** (Visibility, Competitive, Narrative, Sources, Prompts, Recommendations) — spoke nav fully lit, no "Soon" left. Recommendations (`subjects/[id]/recommendations/page.tsx`) = recommended-actions card (primary + secondary + warning) with a **Regenerate** button (client `RegenerateButton` + `regenerateRecommendedAction` server action → `regenerateRecommendedActions` + revalidate; useTransition pending) + topic battleground (`topic_leaderboard`, biggest-gap-first, you-vs-leader bars). **Remaining: (1)** second pass to deepen each spoke's first SLICE (tracked in each page's on-page `.deferred-note`). DEEPENING IN PROGRESS — **Visibility**: added platform×topic mention-rate heatmap (bronze-intensity cells) + cross-platform divergence (alignment stat strip + divergent-prompt ✓/✕ chips); only rank-distribution-by-platform/topic dropdowns (interactive) remain there. **Narrative FULLY DEEPENED** (deferred-note removed): added narrative-score trend sparklines (directional_lean / criticism_severity / certainty / net_sentiment, reusing `Sparkline` + `trend-tile`) + sentiment-by-platform (reusing `tsm-list`). Competitive/Sources still have their deferred bits; **(2)** Overview evidence "show full quote" expand (`getPromptResponses` wired) + refresh-history disclosure; **(3)** Phase 3 cutover (`git mv web web-old && git mv web-next web`). |
