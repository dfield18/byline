...
| **Remaining** | Overview brief + spoke sidebar done. **Next: port the spokes themselves**, starting with **Visibility** — each becomes a `subjects/[id]/{spoke}/page.tsx`, and you flip its entry in `SPOKES` (components/dashboard/Sidebar.tsx) to `built:true` so the nav links + active-highlight light up. Spoke order from the sidebar: Visibility → Competitive → Narrative → Sources → Prompts → Recommendations. Minor Overview leftovers: lazy "show full quote" expand on evidence cards (`getPromptResponses` already wired) + refresh-history disclosure. |
