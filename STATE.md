...
| **Remaining on `/subjects/[id]`** | Port the rest of the old 2,248-line Overview, each its own decomposition: narrative clusters, competitive landscape, source mix, evidence quotes; the polling **refresh button**; the subject-scoped **spoke sidebar** (nested `subjects/[id]` layout). The on-page `.deferred-note` tracks this boundary visibly. Then the other spokes (Visibility, Narrative, Competition, Sources, Prompts, Recommendations). |
