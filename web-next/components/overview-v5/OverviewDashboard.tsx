...
export interface KpiMetric {
  ...
  spark?: (number | null)[];
  /** Optional 0–100 share, rendered as a small bar (used when there's no spark). */
  barPct?: number;
  /** Marks the headline KPI for stronger visual emphasis (larger value). */
  primary?: boolean;
...
export interface OverviewData {
  ...
  /** At-a-glance status chip by the subject name (derived from the mention delta). */
  statusChip?: { label: string; tone: "warn" | "good" | "neutral" } | null;
  updatedLabel: string;
  ...
  summaryRest: string;
  /** "Why it matters" bridge line, shown under the KPI strip. */
  whyItMatters?: string | null;
  /** Interpretation sentence under the Mention rate trend chart. */
...
function Sparkline({
  ...
  const y0 = TY;
  const y1 = H - BY;
  // Tighten the vertical domain to the data band (mention rate clusters high),
  // rounded to nice 10s, so the plot isn't mostly empty below the data.
  const allVals = drawable.flatMap((s) => s.points.filter((v): v is number => v != null));
  const dataMin = allVals.length ? Math.min(...allVals) : 0;
  const dataMax = allVals.length ? Math.max(...allVals) : 100;
  const domainMin = Math.max(0, Math.floor(dataMin / 10) * 10 - 10);
  const domainMax = Math.min(100, Math.max(domainMin + 20, Math.ceil(dataMax / 10) * 10));
  const N = labels.length;

  const xAt = (i: number) => (N <= 1 ? x1 : x0 + (i / (N - 1)) * (x1 - x0));
  const yAt = (v: number) =>
    y1 - ((Math.max(domainMin, Math.min(domainMax, v)) - domainMin) / (domainMax - domainMin)) * (y1 - y0);
  const clampY = (y: number) => Math.max(y0, Math.min(y1, y));
  // Gentle Catmull-Rom (low tension) — just rounds the corners between data
  // points without the dramatic over/undershoot of a full-tension spline.
  const TENSION = 0.5; // 0 = straight segments, 1 = full Catmull-Rom
  ...
  // Rivals are background context — keep them light/muted so the subject line
  // is clearly dominant (not 4 equally-weighted lines).
  const RIVAL_STROKE = "#c4c8ce";
  const rivalOpacity = (i: number) => (i === 0 ? 0.7 : 0.55);
  ...
  return (
    ...
    >
      ...
      {/* gridlines + y-axis ticks — at the tightened domain's min / mid / max */}
      {[domainMin, Math.round((domainMin + domainMax) / 2), domainMax].map((g) => (
        <g key={g}>
      ...
      {drawable.map((s) => {
        ...
        return (
          <path
            ...
            strokeWidth={s.isSubject ? 3.4 : 1.5}
            strokeLinecap="round"
      ...
      {/* data-point dots on the subject line only (the hero); rivals stay clean */}
      {subj &&
        (() => {
          const li = lastDefined(subj.points);
          return subj.points.map((v, i) =>
            v == null ? null : (
              <circle
                key={`subj-dot-${i}`}
                cx={xAt(i)}
...
// Tiny position-on-a-range cue (e.g. sentiment on −1…+1) — frames a lone number.
function KpiScale({ value, min, max }: { value: number; min: number; max: number }) {
  ...
  return (
    <div className="bo-kscale" aria-hidden>
      ...
      <span className="bo-kscale-zero" style={{ left: `${zero}%` }} />
      <span className="bo-kscale-dot" style={{ left: `${pct}%` }} />
    </div>
  );
}
  ...
  return (
    <div className="byline-overview">
      ...
      <header className="bo-head">
        ...
        {data.statusChip && (
          <span className={`bo-status bo-status--${data.statusChip.tone}`}>
      ...
      <div className="bo-kpis">
        {data.kpis.map((k) => (
          <div key={k.id} className={`bo-kpi${k.primary ? " bo-kpi--primary" : ""}`}>
            ...
            <div className="bo-kpi-body">
              ...
              {k.scale ? (
                ...
                <div className="bo-kpi-viz">
                  <span className="bo-kpi-bar" aria-hidden>
                    <span
                      className="bo-kpi-bar-fill"
                      style={{ width: `${Math.max(0, Math.min(100, k.barPct))}%` }}
                    />
                  </span>
                </div>
              ) : null}
      ...
      {data.whyItMatters && (
        <p className="bo-why">
          <span className="bo-why-k">Why it matters:</span> {data.whyItMatters}
        </p>
      )}
      ...
      <div className="bo-toprow">
        ...
        <section className="bo-card">
          ...
          />
          {data.trendNote && <p className="bo-trendnote">{data.trendNote}</p>}
        </section>
      ...
      <div className="bo-diag">
        ...
        {/* Topic coverage — grouped by association strength; placed first since
            topic gaps are the most directly actionable part of the diagnosis. */}
        <section className="bo-card">
          ...
          {data.coverage.rows.length > 0 ? (
            (() => {
              const order: Record<Association, number> = { strong: 0, moderate: 1, weak: 2, missing: 3 };
              const sorted = [...data.coverage.rows].sort((a, b) => order[a.level] - order[b.level]);
              const groups = [
                {
                  label: "Strong associations",
                  rows: sorted.filter((r) => r.level === "strong" || r.level === "moderate"),
                },
                {
                  label: "Weak or missing associations",
                  rows: sorted.filter((r) => r.level === "weak" || r.level === "missing"),
                },
              ...
              return (
                <>
                  <div className="bo-ptm bo-ptm--nobreak">
                    {groups.map((g) => (
                      <div className="bo-tcgroup" key={g.label}>
                        <div className="bo-tcgroup-label">{g.label}</div>
                        {g.rows.map((row) => (
                  ...
                  <p className="bo-ptm-legend">Association = link strength across the tracked topics.</p>
                </>
              );
            })()
          ) : (
        ...
        <section className="bo-card">
          ...
          <div className="bo-readout">
            {/* Header eyebrow — mirrors Topic coverage's THEME · ASSOCIATION. */}
            <div className="bo-readout-head">
              <span>Model</span>
              <span>Frame</span>
            </div>
            {data.models.map((m) => {
...
/* At-a-glance status chip by the subject name. */
.bo-status { align-self: center; font-size: 9.5px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.04em; padding: 3px 9px; border-radius: 6px; white-space: nowrap; }
.bo-status--warn { background: var(--bo-neu-bg); color: var(--bo-neu); }
.bo-status--good { background: var(--bo-pos-bg); color: var(--bo-pos); }
.bo-status--neutral { background: var(--bo-sand); color: var(--bo-muted); }
...
.bo-comparison { margin: 0 0 12px; font-size: 11px; color: var(--bo-muted); }
/* "Why it matters" bridge line under the KPI strip — subtle, semibold label. */
.bo-why { margin: 0 0 26px; font-size: 12.5px; line-height: 1.5; color: var(--bo-ink-soft); max-width: 820px; }
.bo-why-k { font-weight: 700; color: var(--bo-bronze-deep); }
...
/* Primary (headline) KPI — larger value for stronger emphasis. */
.bo-kpi--primary .bo-kpi-value { font-size: 31px; }
.bo-kpi--primary .bo-kpi-delta { font-size: 13.5px; }
...
/* Share bar — fills the KPI viz slot for the Top Cited Source (no trend). */
.bo-kpi-bar { display: block; width: 100%; height: 8px; border-radius: 5px; background: var(--bo-sand); overflow: hidden; }
.bo-kpi-bar-fill { display: block; height: 100%; border-radius: 5px; background: var(--bo-bronze); opacity: 0.55; }
/* Position-on-a-range cue (sentiment −1…+1): track + zero tick + value dot. */
.bo-kscale { position: relative; width: 100%; height: 16px; }
.bo-kscale-track { position: absolute; top: 50%; left: 0; right: 0; height: 3px; margin-top: -1.5px; border-radius: 2px; background: var(--bo-sand); }
.bo-kscale-zero { position: absolute; top: 50%; width: 1px; height: 11px; margin-top: -5.5px; background: var(--bo-line-strong); transform: translateX(-50%); }
.bo-kscale-dot { position: absolute; top: 50%; width: 9px; height: 9px; margin-top: -4.5px; border-radius: 50%; background: var(--bo-bronze); transform: translateX(-50%); }
...
.bo-card--action { background: linear-gradient(180deg, #faf6ec 0%, #fffdf8 72%); border-color: #f2ece1; box-shadow: inset 4px 0 0 var(--bo-bronze), var(--bo-shadow); padding: 18px 24px; }
.bo-card--action .bo-rec-num { background: var(--bo-bronze); color: #fff; }
.bo-card--action .bo-cardhead { margin-bottom: 9px; }
...
/* Priority + What-changed share one compact 2-up intro row (stack when alone). */
.bo-intro { display: grid; grid-template-columns: 1fr 1fr; gap: 6px 26px; align-items: start; margin-bottom: 13px; }
.bo-intro > :only-child { grid-column: 1 / -1; }
.bo-intro .bo-cardnote { margin: 0; }
.bo-intro .bo-changed { margin: 0; }
...
/* The cards are equal height (stretch); let the shorter card's content grow so
   it reaches the same bottom — the Topic coverage rows fill the extra space. */
.bo-diag-row > .bo-card { display: flex; flex-direction: column; }
.bo-diag-row .bo-ptm { flex: 1; justify-content: space-between; }
.bo-diag-row .bo-readout { flex: 1; }
/* Topic coverage groups (Strong vs Weak/Missing). */
.bo-tcgroup { display: flex; flex-direction: column; }
.bo-tcgroup + .bo-tcgroup { margin-top: 10px; }
.bo-tcgroup-label { font-size: 10px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.05em; color: var(--bo-muted); padding: 4px 0 3px; }
/* Cross-model readout — mirrors the landing console's .lc-card rows:
   logo badge · (name + sentiment + frame) · evidence line, stacked vertically. */
.bo-readout { display: flex; flex-direction: column; }
/* Header eyebrow — parallel to Topic coverage's THEME · ASSOCIATION row. */
.bo-readout-head { display: flex; justify-content: space-between; padding: 0 0 10px; font-size: 10px; text-transform: uppercase; letter-spacing: 0.05em; color: var(--bo-muted); font-weight: 700; }
/* Right-aligned "assessment" column (sentiment + frame), like the badge column. */
.bo-rassess { margin-left: auto; display: inline-flex; align-items: center; gap: 8px; flex: none; }
...
.bo-rec-title { font-size: 14px; font-weight: 700; letter-spacing: -0.01em; line-height: 1.25; color: var(--bo-ink); }
...
.bo-rec-why { margin: 6px 0 0; font-size: 12px; line-height: 1.45; color: var(--bo-muted); display: -webkit-box; -webkit-line-clamp: 2; line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; }
...
.bo-rec-addr { margin: 8px 0 0; font-size: 10px; line-height: 1.35; color: #90909a; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
...
.bo-recs--cols .bo-rec { align-items: flex-start; gap: 10px; padding: 15px 16px; border: 1px solid var(--bo-line); border-radius: 12px; background: #fdfbf6; }
...
.bo-changed { display: flex; align-items: flex-start; gap: 8px; margin: 0 0 13px; }
...
/* Interpretation sentence under the Mention rate trend chart. */
.bo-trendnote { margin: 12px 2px 0; font-size: 12px; line-height: 1.45; color: var(--bo-muted); }
...
@media (max-width: 860px) {
  ...
  .bo-toprow, .bo-grid, .bo-diag-row, .bo-intro { grid-template-columns: 1fr; }
  .bo-recs--cols { grid-template-columns: 1fr 1fr; }
  .bo-intro .bo-cardnote { margin: 0 0 4px; }
}
