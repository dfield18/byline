...
export function toOverviewData(api: SubjectOverview): OverviewData {
  ...
  // Subject's prominence (how often it's the TOP-ranked answer, + avg rank).
  const subjRow = api.competitive.find((c) => c.is_subject);
  const topAnswerPct =
    subjRow && subjRow.first_mention_rate != null ? Math.round(subjRow.first_mention_rate * 100) : null;
  const subjAvgRank = subjRow?.avg_rank ?? null;
  // Order: visibility/prominence/source-control first, sentiment last.
  const kpis: KpiMetric[] = [
    {
      ...
      spark: t.ai_recall,
      primary: true, // the headline visibility metric — stronger emphasis
    ...
    },
    {
      ...
      value: topAnswerPct === null ? "—" : `${topAnswerPct}%`,
      delta: subjAvgRank != null ? `avg rank ${subjAvgRank.toFixed(1)}` : "no prior",
      deltaDirection: "neutral",
      info: "Share of answers where this subject is the top-ranked entity named — not just mentioned, but led.",
      interpretation:
        topAnswerPct === null
          ? "No ranking data yet"
          : topAnswerPct >= 50
            ? "Often the lead answer"
            : topAnswerPct >= 20
              ? "Sometimes the lead answer"
              : "Rarely the lead answer",
    },
    {
      id: "citation",
      ...
      info: "Share of AI answers citing owned, official, or controlled sources.",
    ...
    },
    {
      ...
      // Scale cue (−1…+1) instead of a sparkline — frames where the value sits.
      scale:
        k.avg_sentiment.value === null
          ? undefined
          : { value: k.avg_sentiment.value, min: -1, max: 1 },
  ...
  // Status chip by the subject name — an at-a-glance verdict from the mention-
  // rate move vs the prior run. Amber = declining, green = rising, gray = stable.
  const recallDeltaPp = k.ai_recall.delta != null ? Math.round(k.ai_recall.delta * 100) : null;
  const statusChip =
    recallDeltaPp === null
      ? null
      : recallDeltaPp <= -5
        ? { label: "Visibility declining", tone: "warn" as const }
        : recallDeltaPp >= 5
          ? { label: "Visibility rising", tone: "good" as const }
          : { label: "Visibility stable", tone: "neutral" as const };
  ...
  // Short name (last token) — the full name already appears directly above.
  const shortName = api.subject_name.split(/\s+/).slice(-1)[0] || api.subject_name;
  ...
  // Executive diagnosis (derived): where the subject is visible, where it's
  // losing ground, and the opening — reusable for any subject from its data.
  const strongAssoc = drivers
    .filter((d) => d.association === "strong" || d.association === "moderate")
    .map((d) => d.label);
  const losingThemes = [...missingThemes, ...weakThemes];
  const declining = recallDeltaPp != null && recallDeltaPp < 0;
  const diagWhere = strongAssoc.length
    ? `visible on ${listNames(strongAssoc.slice(0, 2))}`
    : "thinly represented across the tracked topics";
  const diagLose = losingThemes.length
    ? `, but ${declining ? "losing ground" : "absent"} on ${listNames(losingThemes.slice(0, 2))}`
    : "";
  const diagOpp = topRivals.length
    ? ` The opening is to rebuild association where ${listNames(topRivals)} now lead.`
    : "";
  const summaryRest = `${api.subject_name} is ${diagWhere}${diagLose}.${diagOpp}`;

  // "Why it matters" — bridges the KPI decline → competitive landscape → recs.
  const whyItMatters = topRivals.length
    ? `Models still mention ${shortName}, but increasingly rank ${listNames(topRivals)} ahead in leadership-related prompts.`
    : null;

  // Trend interpretation — ranks well when mentioned, but inclusion is the gap.
  const trendNote =
    subjAvgRank != null && mentionPct != null
      ? `${shortName} ranks ${subjAvgRank.toFixed(1)} when mentioned, but appears in only ${mentionPct}% of answers — the gap is inclusion, not ranking.`
      : null;
  // Concise one-liner — the specific themes are enumerated in the rows below,
