...
export function toOverviewData(api: SubjectOverview): OverviewData {
  ...
  const kpis: KpiMetric[] = [
    ...
    },
    {
      ...
      delta: topSrc ? `cited in ${Math.round(topSrc.response_coverage * 100)}% of answers` : "no citations",
  ...
  const summaryRest =
    mentionPct === null
      ? ""
      : `${shortName} appears in only ${mentionPct}% of answers and is framed as a state-level figure while rivals dominate national GOP leadership prompts.`;
