...
function Sparkline({
  ...
  const rivalOpacity = (i: number) => (i === 0 ? 0.9 : 0.72);
    ...
    >
      ...
      {annotation &&
        (() => {
          ...
          return (
            <text className="bo-spark-annot" x={x1 + 8} y={Math.min(subjLabel.y + 20, H - 3)}>
              {annotation}
            </text>
          );
        })()}
    ...
    <div className="byline-overview">
      ...
      <div className="bo-grid">
        ...
        <section className="bo-card">
          ...
          {data.themesSummary && <p className="bo-insight">{data.themesSummary}</p>}
          {data.coverage.rows.length > 0 ? (
            <div className="bo-sb">
              <div className="bo-sb-row bo-sb-axis">
                <span className="bo-sb-theme" />
                <span className="bo-sb-ticks">
                  <span>Missing</span>
                  <span>Weak</span>
                  <span>Moderate</span>
                  <span>Strong</span>
                </span>
                <span className="bo-sb-level" />
              </div>
              {data.coverage.rows.map((row) => (
                <div
                  className="bo-sb-row"
                  key={row.id}
                  title={`${row.full} — ${ASSOC_LABEL[row.level]} coverage`}
                >
                  <span className="bo-sb-theme">{row.label}</span>
                  <span
                    className="bo-sb-track"
                    role="img"
                    aria-label={`${row.label}: ${ASSOC_LABEL[row.level]} coverage`}
                  >
                    <span className="bo-sb-grid" aria-hidden />
                    <span
                      className="bo-sb-fill"
                      style={{ width: `${LEVEL_FRAC[row.level] * 100}%`, background: LEVEL_COLOR[row.level] }}
                    />
                  </span>
                  <span className={`bo-sb-level bo-sb-level--${row.level}`}>{ASSOC_LABEL[row.level]}</span>
                </div>
              ))}
            </div>
          ) : (
        ...
        <section className="bo-card">
          ...
          {data.framingInsight && <p className="bo-insight">{data.framingInsight}</p>}
...
.bo-benchmark { display: inline-flex; align-items: baseline; gap: 5px; padding: 2px 9px; border-radius: 999px; background: rgba(138,109,47,0.08); white-space: nowrap; }
.bo-benchmark-k { font-size: 9.5px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.04em; color: var(--bo-muted); }
.bo-benchmark-v { font-size: 11.5px; font-weight: 700; font-variant-numeric: tabular-nums; color: var(--bo-bronze-deep); }
...
.bo-spark-label { font-size: 14.5px; letter-spacing: -0.01em; }
.bo-spark-annot { fill: var(--bo-muted); font-size: 11px; }
...
.bo-rcard { display: flex; gap: 14px; padding: 14px 0; border-top: 1px solid var(--bo-line); }
...
/* Compact evidence cue tying an action back to the diagnosis below. */
.bo-rec-addr { margin: 9px 0 0; font-size: 11px; line-height: 1.4; color: var(--bo-muted); }
.bo-rec-addr-k { font-weight: 700; color: var(--bo-ink-soft); }
...
.bo-changed { display: flex; align-items: flex-start; gap: 8px; margin: 0 0 16px; }
...
.bo-ptm-row { display: grid; grid-template-columns: minmax(0,1fr) repeat(var(--bo-ptm-cols, 2), 46px) 96px; align-items: center; gap: 10px; padding: 9px 8px; margin: 0 -8px; border-top: 1px solid var(--bo-line); border-radius: 7px; }
.bo-ptm-head { border-top: none; padding-bottom: 6px; font-size: 10px; text-transform: uppercase; letter-spacing: 0.05em; color: var(--bo-muted); font-weight: 700; }
.bo-ptm-theme { min-width: 0; font-size: 12.5px; font-weight: 600; color: var(--bo-ink-soft); }
...
.bo-sb-row { display: grid; grid-template-columns: minmax(0, 1fr) 1.25fr 64px; gap: 18px; align-items: center; padding: 13px 0; border-top: 1px solid var(--bo-line); }
...
.bo-ptm-legend { margin: 10px 2px 0; font-size: 11px; line-height: 1.5; color: var(--bo-muted); }
